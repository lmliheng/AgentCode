  ...
  server.registerTool(
    ...
    async ({
      ...
      try {
        ...
        if (containerPort !== undefined) args.push('--containerPort', String(containerPort))
        if (dockerfile) args.push('--dockerfile', dockerfile)
        if (envParams) args.push('--envParams', envParams)
        if (releaseType) args.push('--releaseType', releaseType)
        if (remark) args.push('--remark', remark)
  ...
  server.registerTool(
    ...
    async ({ envId, serviceName, version, detach, region }) => {
      try {
        ...
        if (detach !== false) args.push('--detach')
        // MCP 场景无交互，始终跳过确认
        args.push('--noConfirm')
        if (region) args.push('--region', region)
