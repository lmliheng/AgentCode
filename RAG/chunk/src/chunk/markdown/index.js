 ...
 *                  }
 */
export function markdown_chunk(fileName, content, options = {
    chunkMaxLength: 120,
    chunkOverlapLength: 40 // overlap 可以减少上下文被切断的问题。
}) {
    try {
        let parse = parseMarkdown(fileName, content);
...
export function createChunks(document, options) {
    ...
    return chunks.map((content, index) => {
        ...
        const contentHash = sha256(content).slice(0, 12);
        return {
            chunkId: toChunkId(document.fileName, document.sourceVersion, chunkIndex, content),
            // Chunk 的正文内容。
