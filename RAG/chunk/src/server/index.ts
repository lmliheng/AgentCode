...
import { markdown_chunk, type Chunk } from '../chunk/markdown/index.js'
...
export function createServer(config: serverConfig) {
     ...
     *  @内存存储milvus客户端对象
     */
    let Milvus_client: MilvusClient | undefined = undefined
     ...
     */
    app.post<{}, any, UploadBody>('/md_chunk', upload.single('file'), async (req, res) => {
        try {
            if (!req.file) {
     ...
     */
    app.post<{}, any, UploadBody>('/entity', upload.single('file'), async (req, res) => {
        try {
            if (!req.file) {
                return res.status(400).json({ error: '没有收到文件' })
            }
            ...
            const { originalname, buffer } = req.file
            const content = buffer.toString('utf-8')
            let chunks: Chunk[] = markdown_chunk(originalname, content)
            for (let i = 0; i < chunks.length; i += 64) {
                const batch = chunks.slice(i, i + 64)
                const input = batch.map(chunk => chunk.content)
                const embeddings: number[][] = await createEmbeddings(input, 256)
     ...
     * 4. 返回 TopK 结果。
     */
    async function searchQuestion(client: MilvusClient, question: string, filter: string) {
